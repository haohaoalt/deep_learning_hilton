        ...
        for i in gpu_id_list:
            if i == cur_state[0]:
                if not get_return:
                    ...
                    gpu_space_available[i] = int("".join(list(filter(str.isdigit, cur_state[3])))) - int(
                        "".join(list(filter(str.isdigit, cur_state[2]))))
    if get_return:
...
class LfFColoredMNIST(Dataset):
    def __init__(self, dataset_name, split, train_corr, severity, transform=None, target_transform=None):
        ...
        self.data = np.load(
            os.path.join(IMAGE_PATH, f'{dataset_name}-{skewed_str}-Severity{severity}/{split}/images.npy'))
        ...
        if split != 'test':
            ...
            if len(conflict_data) > 0 and len(conflict_targets) > 0 and len(conflict_biased_targets) > 0:
                ...
                self.conflict_data, self.conflict_targets, self.conflict_biased_targets = torch.stack(
                    img_list), torch.LongTensor(target_list), torch.LongTensor(bias_list)
...
# `Dataloader` 将在 `train_rbm` 等函数中以 `for` 循环的形式被调用.
    def prepare_dataloader(self, dataset_name):
        if dataset_name == 'MNIST':
            ...
            train_dataset = torchvision.datasets.MNIST(root=self.args.data_dir, train=True, transform=trsf,
                                                       download=True)
            train_loader = DataLoader(train_dataset, batch_size=self.args.batch_size, num_workers=self.args.num_workers,
                                      pin_memory=True)
            test_dataset = torchvision.datasets.MNIST(root=self.args.data_dir, train=False, transform=trsf,
                                                      download=True)
    ...
    def prepare_optimizer(self, model):
        def set_optimizer(optimizer_name, cur_encoder):
            if optimizer_name == 'adam':
                ...
                return optim.SGD(model.parameters(), lr=self.args.lr_ae, weight_decay=self.args.weight_decay,
                                 momentum=0.9)
